...
exports.run =  async(client, message, args) => {
  ...
  if (args.join('').toLowerCase() === 'queue') {
      if (queue.repeatMode === 1) return message.channel.send(`You must first disable the current music in the loop mode (${config.app.px}loop) ${message.author}... try again ? ❌`);
      ...
      if (queue.repeatMode === 2) return message.channel.send(`You must first disable the current queue in the loop mode (${config.app.px}loop queue) ${message.author}... try again ? ❌`);
...
exports.conf = {
  ...
  exports.help = {
    name: "loop",
    category: "Music",
    description: "loop the song/queue playing",
    usage: "loop"
  };
